...
import { Search, Check, X, Ban, Trash2, Eye } from "lucide-react";
...
export default function JobsManager({ initialData }: { initialData: Data }) {
  ...
  function refresh(opts?: {
    ...
    startTransition(async () => {
      const d = await getAdminJobListings({
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <select
          ...
          onChange={(e) => {
            ...
            refresh({ type: e.target.value });
          }}
          className={inputClass}
        >
          <option value="all">Todos os tipos</option>
          <option value="oferta">Oferece vaga</option>
          <option value="candidato">Procura vaga</option>
        </select>
        <select
          value={status}
          onChange={(e) => {
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="text-left text-white/40 border-b border-white/10">
              <th className="pb-3 font-medium">Publicação</th>
