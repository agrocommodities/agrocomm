...
export default function CalculadoraPecuariaComercial({ quotes }: Props) {
  ...
  const result = useMemo(() => {
    ...
    const valueLiveWeight = (
      animals: number,
      liveWeightKg: number,
      pricePerKg: number,
      premiumPercent = 0,
    ) => animals * liveWeightKg * pricePerKg * (1 + premiumPercent / 100);
    const valueSlaughter = (
      animals: number,
      liveWeightKg: number,
      yieldPercent: number,
      arroba: number,
    ) => animals * ((liveWeightKg * (yieldPercent / 100)) / 15) * arroba;
    const calculateCommercialCosts = (revenue: number, slaughter: boolean) => {
      ...
      if (modalidadeVenda === "leilao")
        return freteLeilao + revenue * (comissaoLeilao / 100) + invoiceCost;
    ...
    const buildScenario = (
      ...
      return {
        ...
        monthlyRevenue: revenue / totalMonths,
        monthlyCosts: costs / totalMonths,
        monthlyProfit: profit / totalMonths,
      };
    };

    const calfRevenue =
      valueLiveWeight(males, weights.calfMale, precoKgVivoMacho, agioLeilao) +
      valueLiveWeight(
        females,
        weights.calfFemale,
        precoKgVivoFemea,
    ...
    const youngRevenue =
      valueLiveWeight(males, weights.youngMale, precoKgVivoMacho, agioLeilao) +
      valueLiveWeight(
        females,
        weights.youngFemale,
        precoKgVivoFemea,
    ...
    const breederRevenue =
      valueLiveWeight(males, weights.adultMale, precoKgVivoMacho) +
      valueLiveWeight(
        females,
        weights.adultFemale,
        precoKgVivoFemea,
    ...
    const slaughterRevenue =
      valueSlaughter(males, weights.adultMale, rendimentoMacho, arrobaBoi) +
      valueSlaughter(females, weights.adultFemale, rendimentoFemea, arrobaVaca);
        ...
        ? [
            buildScenario(
              ...
              modalidadeVenda === "leilao"
                ? "Gado vivo vendido por kg, com frete, comissão e nota fiscal."
                : "Gado vivo vendido por kg, com corretagem e nota fiscal.",
              8,
              calfRevenue,
            ),
            buildScenario(
              ...
              modalidadeVenda === "leilao"
                ? "Gado vivo por kg após gestação + 24 meses, com custos de leilão."
                : "Gado vivo por kg após gestação + 24 meses, com corretagem e nota fiscal.",
              24,
              youngRevenue,
            ),
            buildScenario(
              ...
              modalidadeVenda === "leilao"
                ? "Animais vivos por kg, com valorização reprodutiva e custos de leilão."
                : "Animais vivos por kg, com valorização reprodutiva, corretagem e nota fiscal.",
              36,
              breederRevenue,
            ),
            buildScenario(
              "abate",
              "Bovinos para abate",
              "Venda por arroba de carcaça, com rendimento e nota fiscal.",
              36,
              slaughterRevenue,
              true,
            ),
    ...
    const revenue = slaughter
      ? valueSlaughter(animals, pesoSaida, rendimentoMacho, arrobaBoi)
      : valueLiveWeight(animals, pesoSaida, precoKgVivoMacho, agioLeilao);
    const commercialCosts = calculateCommercialCosts(revenue, slaughter);
    const totalCosts =
      recurringCosts + fixedCosts + purchaseCosts + commercialCosts;
    ...
    return {
      ...
      single: {
        ...
        profit,
        margin: revenue > 0 ? (profit / revenue) * 100 : 0,
        monthlyRevenue: revenue / safeMonths,
        monthlyCosts: totalCosts / safeMonths,
        monthlyProfit: profit / safeMonths,
      },
    };
  }, [
    sistema,
  ...
  return (
    <div className="flex min-w-0 flex-col gap-6">
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          <label className="flex min-w-0 flex-col gap-1.5">
            ...
            >
              ...
              <option value="recria">Recria — venda de gado vivo</option>
              <option value="engorda">Engorda — venda para abate</option>
            </select>
          </label>
          <label className="flex min-w-0 flex-col gap-1.5">
            <span className="text-xs font-medium text-white/55">
      ...
      <div className="grid min-w-0 gap-6 xl:grid-cols-[1.05fr_0.95fr]">
        <div className="flex min-w-0 flex-col gap-6">
          ...
          <section className="rounded-2xl border border-white/10 bg-white/5 p-4 sm:p-6">
            ...
            <p className="mb-4 text-xs text-white/35">
              O abate considera somente nota fiscal. As vendas de gado vivo usam
              a modalidade escolhida.
            </p>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              <label className="flex min-w-0 flex-col gap-1.5">
                <span className="text-xs font-medium text-white/55">
                  Modalidade das vendas de gado vivo
                </span>
                <select
                  value={modalidadeVenda}
                  onChange={(event) =>
                    setModalidadeVenda(event.target.value as ModalidadeVenda)
                  }
                  className="w-full min-w-0 rounded-xl border border-white/10 bg-[#151a13] px-3 py-2.5 text-sm outline-none"
                >
                  <option value="leilao">Leilão</option>
                  <option value="pecuarista">
                    Venda para outro pecuarista
                  </option>
                </select>
              </label>
              {modalidadeVenda === "leilao" ? (
                <>
                  <Field
                    label="Frete para o leilão"
                    value={freteLeilao}
                    onChange={setFreteLeilao}
                    suffix="R$ total"
                  />
                  <Field
                    label="Comissão do leilão"
                    value={comissaoLeilao}
                    onChange={setComissaoLeilao}
                    suffix="% receita"
                    step={0.1}
                    max={100}
                  />
          ...
          <section className="rounded-2xl border border-white/10 bg-white/5 p-4 sm:p-6">
            <h2 className="mb-1 text-lg font-semibold">Preços de venda</h2>
            <p className="mb-4 text-xs text-white/35">
              Gado vivo é calculado por kg vivo. Somente o abate utiliza arroba
              e rendimento de carcaça.
            </p>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              <Field
                label="Preço do kg vivo — machos"
                value={precoKgVivoMacho}
                onChange={setPrecoKgVivoMacho}
                suffix="R$/kg"
                step={0.01}
              />
              <Field
                label="Preço do kg vivo — fêmeas"
                value={precoKgVivoFemea}
                onChange={setPrecoKgVivoFemea}
                suffix="R$/kg"
                step={0.01}
              />
              <Field
          ...
          <section className="rounded-2xl border border-white/10 bg-white/5 p-4 sm:p-6">
            <h2 className="mb-1 text-lg font-semibold">Preços para abate</h2>
            <p className="mb-4 text-xs text-white/35">
              As cotações do site são usadas somente nos cenários de abate.
            </p>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              <label className="flex min-w-0 flex-col gap-1.5">
                <span className="text-xs font-medium text-white/55">
          ...
          {sistema === "cria" ? (
            <>
              <section className="rounded-2xl border border-white/10 bg-white/5 p-4 sm:p-6">
                ...
                <p className="mt-1 text-xs text-white/35">
                  Vendas vivas usam R$/kg vivo; somente o abate usa R$/arroba.
                </p>
                  ...
                  {result.scenarios.map((scenario) => (
                    ...
                    >
                      ...
                      <p className="mt-3 text-xs text-white/35">
                        Ciclo total: {decimal.format(scenario.totalMonths)}{" "}
                        meses
                      </p>
                      <p
                      ...
                      <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
                        <div>
                          <span className="text-white/35">Receita total</span>
                          <p>{currency.format(scenario.revenue)}</p>
                        </div>
                        <div>
                          <span className="text-white/35">Custo total</span>
                          <p>{currency.format(scenario.costs)}</p>
                        </div>
                        <div>
                          <span className="text-white/35">Produção</span>
                          <p>{currency.format(scenario.productionCosts)}</p>
                        </div>
                        <div>
                          <span className="text-white/35">Comercialização</span>
                          <p>{currency.format(scenario.commercialCosts)}</p>
                        </div>
                        <div>
                        ...
                        <div className="mt-2 grid grid-cols-3 gap-2 text-xs">
                          <div>
                            ...
                            <p className="font-semibold text-green-400">
                              {currency.format(scenario.monthlyRevenue)}
                            </p>
                          </div>
                          <div>
                            <span className="text-white/35">Gastos</span>
                            <p className="font-semibold text-red-400">
                              {currency.format(scenario.monthlyCosts)}
                            </p>
                          </div>
                          <div>
                            <span className="text-white/35">Lucro</span>
                            <p
                              className={`font-semibold ${scenario.monthlyProfit >= 0 ? "text-green-400" : "text-red-400"}`}
                            >
                              {currency.format(scenario.monthlyProfit)}
                            </p>
                          </div>
                        </div>
                      </div>
            ...
            >
              ...
              <p className="mt-2 text-xs text-white/40">
                {sistema === "recria"
                  ? "Receita calculada pelo peso vivo em kg."
                  : "Receita calculada por arroba de carcaça."}
              </p>
              <div className="mt-5 grid grid-cols-2 gap-3 text-sm">
                ...
                <div>
                  <span className="text-white/35">Comercialização</span>
                  <p>{currency.format(result.single.commercialCosts)}</p>
                </div>
                <div>
                  <span className="text-white/35">Custos totais</span>
              ...
              <div className="mt-5 rounded-xl border border-white/10 bg-black/15 p-4">
                ...
                <div className="mt-3 grid grid-cols-3 gap-3 text-sm">
                  <div>
                    ...
                    <p className="font-semibold text-green-400">
                      {currency.format(result.single.monthlyRevenue)}
                    </p>
                  </div>
                  <div>
                    <span className="text-xs text-white/35">Gastos</span>
                    <p className="font-semibold text-red-400">
                      {currency.format(result.single.monthlyCosts)}
                    </p>
                  </div>
                  <div>
                    <span className="text-xs text-white/35">Lucro</span>
                    <p
                      className={`font-semibold ${result.single.monthlyProfit >= 0 ? "text-green-400" : "text-red-400"}`}
                    >
                      {currency.format(result.single.monthlyProfit)}
                    </p>
                  </div>
                </div>
              </div>
      ...
      <p className="text-center text-xs leading-relaxed text-white/30">
        Vendas de gado vivo são calculadas por peso vivo em quilogramas.
        Cenários de abate usam rendimento de carcaça e preço da arroba.
      </p>
    </div>
  );
}
