...
const SYSTEMS: SystemDefinition[] = [
  {
    ...
    months: 17,
    maleExitWeight: 210,
    femaleExitWeight: 190,
    freightCapacity: 25,
    purchaseKind: "matrizes",
    description:
  ...
  },
  {
    ...
    femaleExitWeight: 360,
    freightCapacity: 20,
    purchaseKind: "bezerros",
    description:
      "Compra de bezerros e bezerras para venda como boi, vaca ou garrote.",
  },
  {
    id: "recria-abate",
    ...
    months: 24,
    maleExitWeight: 520,
    femaleExitWeight: 440,
    freightCapacity: 18,
    purchaseKind: "bezerros",
    description: "Compra de bezerros e bezerras e terminação até o abate.",
  },
  {
    id: "engorda-vivo",
    ...
    purchaseKind: "magros",
    description:
      "Compra de boi/vaca ou novilho/novilha magros para venda gordos.",
  },
  {
    id: "engorda-abate",
    ...
    months: 8,
    maleExitWeight: 540,
    femaleExitWeight: 460,
    freightCapacity: 18,
    purchaseKind: "magros",
    description: "Compra de gado magro e venda final ao frigorífico.",
  },
];
...
export default function SistemasProdutivosCalculatorV3({ quotes }: Props) {
  ...
  const cityOptions = useMemo(() => {
    ...
    for (const quote of quotes) {
      ...
      map.set(`${quote.city}|${quote.state}`, {
        city: quote.city,
        state: quote.state,
      });
    }
  ...
  const boiQuote = quotes.find(
    (item) =>
      item.productSlug === "boi-gordo" &&
      item.city === selectedCity &&
      item.state === selectedState,
  );
  const vacaQuote = quotes.find(
    (item) =>
      item.productSlug === "vaca-gorda" &&
      item.city === selectedCity &&
      item.state === selectedState,
  );
  ...
  const calculations = useMemo(() => {
    const calculate = (system: SystemDefinition): Result => {
      ...
      const localFemaleWeight = active
        ? femaleExitWeight
        : system.femaleExitWeight;
      const breedingBase =
        system.group === "Cria" ? quantity * (birthRate / 100) : quantity;
      const sold = breedingBase * (1 - mortality / 100);
      const females = sold * (femalePercent / 100);
      const males = sold - females;

      const saltMonthly =
        ((saltDailyKg * breedingBase * 30) / Math.max(1, saltBagKg)) *
        saltBagPrice;
      const recurring =
        saltMonthly + veterinarianMonthly + workerMonthly + otherMonthlyCosts;
      const healthCosts =
        (vaccineDosePrice * vaccineDoses + dewormerDosePrice * dewormerDoses) *
        breedingBase;
      const purchaseCosts = purchaseUnitCost(system.purchaseKind) * quantity;
      const productionCosts =
        recurring * localMonths + healthCosts + fixedCosts + purchaseCosts;

      const revenue = system.liveSale
        ? males * localMaleWeight * livePriceMale +
          females * localFemaleWeight * livePriceFemale
        : males * ((localMaleWeight * (maleYield / 100)) / 15) * maleArroba +
          females *
            ((localFemaleWeight * (femaleYield / 100)) / 15) *
            femaleArroba;

      const freightTrips =
        system.liveSale && saleChannel === "leilao"
          ? Math.ceil(sold / system.freightCapacity)
          : 0;
      const commercialRate = system.liveSale
        ? saleChannel === "leilao"
          ? auctionCommission
          : brokerCommission
        : 0;
      const commercialCosts =
        revenue * ((commercialRate + invoiceTax) / 100) +
        freightTrips * freightPerTruck;
    ...
    const saltMonthly =
      ((saltDailyKg * base * 30) / Math.max(1, saltBagKg)) * saltBagPrice;
    const recurring =
      saltMonthly + veterinarianMonthly + workerMonthly + otherMonthlyCosts;
    const healthCosts =
      (vaccineDosePrice * vaccineDoses + dewormerDosePrice * dewormerDoses) *
    ...
    const freightTrips =
      selected.liveSale && saleChannel === "leilao"
        ? Math.ceil(sold / selected.freightCapacity)
        : 0;
    const commercialRate = selected.liveSale
      ...
      : 0;
    const commercialCosts =
      current.revenue * ((commercialRate + invoiceTax) / 100) +
      freightTrips * freightPerTruck;
    ...
    const projection = Array.from(
      ...
      (_, month) => {
        ...
        return {
          ...
          ganhos: Math.round(gains),
          resultado: Math.round(gains - costs),
        };
      },
    );
    ...
    return {
      ...
      freightTrips,
      sold,
      males: sold * (malePercent / 100),
      females: sold * (femalePercent / 100),
    };
  }, [
    systemId,
    selected,
    ...
    <div className="flex min-w-0 flex-col gap-6">
      ...
      <div className="grid min-w-0 gap-6 xl:grid-cols-[1.05fr_0.95fr]">
        <div className="flex min-w-0 flex-col gap-6">
          <section className="rounded-2xl border border-white/10 bg-white/5 p-4 sm:p-6">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              ...
              />
              <Field
                label="Peso de venda — machos"
                value={maleExitWeight}
                onChange={setMaleExitWeight}
                suffix="kg vivo"
              />
              <Field
                label="Peso de venda — fêmeas"
                value={femaleExitWeight}
                onChange={setFemaleExitWeight}
                suffix="kg vivo"
              />
            </div>
            <label className="mt-5 flex flex-col gap-2">
              <span className="text-xs font-medium text-white/55">
                Distribuição do lote: {malePercent}% machos e {femalePercent}%
                fêmeas
              </span>
              <input
                type="range"
                ...
                value={femalePercent}
                onChange={(event) =>
                  setFemalePercent(Number(event.target.value))
                }
                className="w-full accent-green-500"
              />
              <div className="flex justify-between text-[11px] text-white/30">
                <span>0% fêmeas</span>
                <span>100% fêmeas</span>
              </div>
            </label>
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              <div className="rounded-xl bg-black/15 p-3">
                <span className="text-xs text-white/35">Machos projetados</span>
                <p className="font-semibold">
                  {decimal.format(calculations.males)}
                </p>
              </div>
              <div className="rounded-xl bg-black/15 p-3">
                <span className="text-xs text-white/35">Fêmeas projetadas</span>
                <p className="font-semibold">
                  {decimal.format(calculations.females)}
                </p>
              </div>
            </div>
          </section>
          ...
          <section className="rounded-2xl border border-white/10 bg-white/5 p-4 sm:p-6">
            <h2 className="mb-1 text-lg font-semibold">
              Custos opcionais de aquisição
            </h2>
            <p className="mb-4 text-xs text-white/35">
              Informe zero quando os animais já pertencem à propriedade.
            </p>
            <div className="grid gap-4 sm:grid-cols-2">
              <Field
                label="Compra de matrizes"
                value={purchaseMatrices}
                onChange={setPurchaseMatrices}
                suffix="R$/cabeça"
              />
              <Field
                label="Compra de bezerros/bezerras"
                value={purchaseCalves}
                onChange={setPurchaseCalves}
                suffix="R$/cabeça"
              />
              <Field
                label="Compra de novilhos/novilhas"
                value={purchaseYoung}
                onChange={setPurchaseYoung}
                suffix="R$/cabeça"
              />
              <Field
                label="Compra de gado magro — vaca/novilho"
                value={purchaseLeanAdult}
                onChange={setPurchaseLeanAdult}
                suffix="R$/cabeça"
              />
            </div>
            <p className="mt-3 text-xs text-white/40">
              Custo aplicado neste sistema:{" "}
              {money.format(calculations.purchaseCosts)}.
            </p>
          </section>
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {selected.liveSale ? (
                ...
                <>
                  ...
                  <Field
                    label="Rendimento de carcaça — machos"
                    value={maleYield}
                    onChange={setMaleYield}
                    suffix="%"
                    step={0.5}
                  />
                  <Field
                    label="Rendimento de carcaça — fêmeas"
                    value={femaleYield}
                    onChange={setFemaleYield}
                    suffix="%"
                    step={0.5}
                  />
                  <div className="rounded-xl bg-black/15 p-3">
                    <span className="text-xs text-white/35">Arroba do boi</span>
                    <p className="font-semibold">
                      {money.format(maleArroba)}/@
                    </p>
                  </div>
                  <div className="rounded-xl bg-black/15 p-3">
        ...
        <div className="flex min-w-0 flex-col gap-6">
          ...
          {[
            {
              title: "Lucro total por modalidade",
              key: "lucro",
              color: "#4ade80",
              suffix: "R$",
            },
            {
              title: "Rentabilidade mensal por modalidade",
              key: "mensal",
              color: "#facc15",
              suffix: "R$",
            },
            {
              title: "Margem percentual por modalidade",
              key: "margem",
              color: "#60a5fa",
              suffix: "%",
            },
