...
import { deleteUserJobListing, pauseUserJobListing } from "@/actions/jobs";
import { Pencil, Trash2, Pause, Play } from "lucide-react";
import Link from "next/link";

export default function JobListingOwnerActions({
  jobId,
  slug,
  status,
}: {
  ...
  function handlePause() {
    startTransition(async () => {
      const result = await pauseUserJobListing(jobId);
  ...
  function handleDelete() {
    ...
    startTransition(async () => {
      const result = await deleteUserJobListing(jobId);
      if (result.success) router.push("/vagas");
    });
  }
